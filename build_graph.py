...
def calculate(words, feature):
    e = np.zeros((len(words), len(words)))
    for row in range(0, len(words)):
        for column in range(row, len(words)):
            if row < column:
                tem = (np.dot(feature[row], feature[column]))/(np.linalg.norm(feature[row])*np.linalg.norm(feature[column]))
                e[row][column] = np.arccos(-tem)
                e[column][row] = e[row][column]
    for i in range(0, len(words)):
        kth_max = np.sort(e[i])
        e_row = e[i]
        e_row = np.where(e_row < kth_max[len(words)-10], 0, e_row)
        e[i] = e_row
    for row in range(0, len(words)):
        for column in range(row, len(words)):
            if e[row][column] != e[column][row]:
                if e[row][column] == 0:
                    e[row][column] = e[column][row]
                if e[column][row] == 0:
                    e[column][row] = e[row][column]

    sparse_e = sparse.csr_matrix(e)
    io.mmwrite("graph", sparse_e)
    return
